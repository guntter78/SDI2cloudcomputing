...
echo "Enter the number of VMs (per node):"
read num_vms
...
read vmname
echo "Enter the starting last octet of the server IP (starting octet):"
read last_octet
...
# Loop door elke node in de cluster_nodes array
for ((node_index=0; node_index<${#cluster_nodes[@]}; node_index++)); do
  dest_node=${cluster_nodes[$node_index]}
  echo "Creating VMs on node ${dest_node}"

  # Voor elke node, verhoog de start VM ID en last octet dynamisch
  vmid_offset=$((start_vmid + (node_index * num_vms)))
  ip_offset=$((last_octet + (node_index * num_vms)))

  # Loop om het opgegeven aantal VM's aan te maken voor elke node
  for ((i=0; i<num_vms; i++)); do
      new_vmid=$((vmid_offset + i))  # Unieke VM ID per node
      new_ip="10.24.36.$((ip_offset + i))"  # Unieke IP per node
